...
- **Configurable workspace directory**: New `tools.exec.workspace` config field (default: `$HOME/workspace`) that separates the agent's working directory from the nanobot config directory (`~/.nanobot`).

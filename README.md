...
```bash
# 不含企业平台
cargo build --release --no-default-features --features "markdown,telegram,discord,slack"
```
...
- `~/.gasket/PROFILE.md` — Agent 角色定义
- `~/.gasket/SOUL.md` — Agent 灵魂
- `~/.gasket/AGENTS.md` — Agent 行为描述
- `~/.gasket/BOOTSTRAP.md` — 启动引导
- `~/.gasket/MEMORY.md` — 长期记忆
- `~/.gasket/hooks/` — Shell Hook 脚本
- `~/.gasket/memory/` — 记忆目录
- `~/.gasket/skills/` — 自定义技能目录
...
# ── 工具配置 ────────────────────────────────────
tools:
  restrict_to_workspace: false  # true = 文件操作限制在 ~/.gasket 内
  exec:
...
# ── 单次对话 ──
gasket agent -m "用 Rust 写一个快速排序"

# ── 交互模式 (REPL) ──
gasket agent

# ── 启用推理模式 (DeepSeek R1 / GLM-5) ──
gasket agent --thinking -m "分析这段代码的时间复杂度"

# ── 禁用流式输出 ──
gasket agent --no-stream -m "hello"

# ── 禁用 Markdown 渲染 ──
gasket agent --no-markdown -m "hello"

# ── 显示调试日志 ──
gasket agent --logs -m "test"
...
在 `~/.gasket/skills/` 目录下创建 Markdown 文件:
...
Gasket-RS 支持在运行时动态切换 LLM 模型。主 Agent 作为编排者，可以将特定任务委托给专门优化的子模型执行。

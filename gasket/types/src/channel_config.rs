...
//!
//! These types were moved from `gasket-channels` to `gasket-types` so that
//! `gasket-engine` can reference channel configuration without depending on
//! the full `gasket-channels` crate (which pulls in adapter implementations).

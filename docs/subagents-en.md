...
Subagent events are forwarded to the WebSocket as `ChatEvent`s:

| ChatEvent | Description |
|-----------|-------------|
| `subagent_started` | Subagent spawned with task description |
| `subagent_thinking` | Subagent's reasoning/thinking content |
| `subagent_tool_start` | Subagent begins using a tool |
| `subagent_tool_end` | Subagent tool call completed |
| `subagent_content` | Content fragment generated by subagent |
...
Subagents can use different models via `model_id`, which references profiles defined in `agents.models`:
...
```rust
// Use a model profile for the subagent
...
agents:
  defaults:
    ...
    subagent_timeout_secs: 600    # Subagent execution timeout (10 min)
    tool_timeout_secs: 120        # Tool execution timeout (2 min)
    ws_summary_limit: 500         # Subagent summary length limit (0 = unlimited)
```

...
MemoryManager 采用三阶段分层加载策略，严格控制 Token 预算：
...
pub struct SessionEvent {
    ...
    pub embedding: Option<Vec<f8>>,  // 用于语义检索
}
...
pub struct AgentSession {
    ...
    pending_done: std::sync::Mutex<Vec<tokio::sync::oneshot::Receiver<()>>>, // 等待完成的任务
}
...
impl AgentSession {
    ...
    pub async fn graceful_shutdown(&self) {
        ...
        if !receivers.is_empty() {
            info!("Graceful shutdown: awaiting {} pending finalization task(s)", receivers.len());
            for rx in receivers {
                let _ = rx.await;
            }
...
│                                                                  │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────────────────┐ │
│  │ SqliteStore  │ │ EventStore   │ │ FileMemoryStore          │ │
│  │ (连接池管理)  │ │ (事件溯源)   │ │ (文件系统记忆)            │ │
│  └──────┬───────┘ └──────┬───────┘ └──────────┬───────────────┘ │
│         │                │                     │                 │
│  ┌──────┴───────┐ ┌──────┴───────┐ ┌──────────┴───────────────┐ │
│  │ MetadataStore│ │EmbeddingStore│ │  RetrievalEngine          │ │
│  │ (记忆元数据)  │ │ (向量索引)   │ │  (语义检索引擎)           │ │
│  └──────────────┘ └──────────────┘ └──────────────────────────┘ │
│                                                                  │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────────────────┐ │
...
// - 记忆管理: MemorizeTool, MemoryRefreshTool, MemoryDecayTool
...
│  │  └───────────┘ └───────────┘ └───────────────────────┘ │  │
│  │  ┌───────────┐ ┌───────────┐ ┌───────────────────────┐ │  │
│  │  │Memory Tools│ │Agent Tools│ │   MCP Tools           │ │  │
│  │  │Search/Save │ │Spawn/Cron │ │(JSON-RPC 2.0 over     │ │  │
│  │  │Decay/Refresh│ │Message   │ │ stdio)                │ │  │
│  │  └───────────┘ └───────────┘ └───────────────────────┘ │  │
│  └─────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
pub struct ToolMetadata {
    pub category: String,           // fs, web, memory, agent, mcp

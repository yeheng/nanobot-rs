...
Gasket 的插件系统让你无需编写 Rust 就能添加自定义工具。只要用你熟悉的语言写一段脚本，配上一个 YAML 声明文件，放到 `~/.gasket/scripts/` 目录下，Gasket 启动时就会自动发现它，并将其作为原生工具暴露给 AI。
...
Gasket 在启动时会扫描 `~/.gasket/scripts/`，将每个 `.yaml` / `.yml` 文件当作插件清单加载。

...
<template>
  <div v-if="hasAnySubagents" class="subagent-panel my-4">
    ...
    <div v-if="runningSubagents.length > 0" class="space-y-2">
      ...
      >
        <div
          class="subagent-header flex items-center gap-2 p-3 cursor-pointer hover:bg-white/5 transition-colors"
        ...
        >
          ...
          />
          <span class="font-medium text-slate-200 text-sm">Task {{ subagent.index }}</span>
          ...
          <span class="text-xs text-slate-500 shrink-0">{{ formatDuration(subagent.startTime) }}</span>
          <component
          ...
          <div v-if="expandedIds.has(subagent.id)" class="subagent-details border-t border-slate-700/50">
            <div class="p-3 pl-6 space-y-2 text-sm">
              ...
              <div v-if="subagent.thinking" class="text-xs">
                <span class="text-slate-500 font-medium">思考：</span>
                <p class="text-slate-400 mt-1 whitespace-pre-wrap break-words">{{ subagent.thinking.slice(0, 300) }}{{ subagent.thinking.length > 300 ? '...' : '' }}</p>
              </div>
              <!-- 输出内容 -->
              <div v-if="subagent.content" class="text-xs">
                <span class="text-slate-500 font-medium">输出：</span>
                <p class="text-slate-300 mt-1 whitespace-pre-wrap break-words">{{ subagent.content.slice(0, 500) }}{{ subagent.content.length > 500 ? '...' : '' }}</p>
              </div>
              <!-- 工具调用 -->
              <div v-if="subagent.toolCalls.length > 0" class="text-xs">
                <span class="text-slate-500 font-medium">工具调用：</span>
                <div class="mt-1 space-y-1">
                  ...
                  >
                    ...
                    <span v-if="tool.duration" class="text-slate-500 text-xs">({{ tool.duration }})</span>
                  </div>
                  <div v-if="subagent.toolCalls.length > 5" class="text-slate-500 pl-5">
    ...
    <div v-if="allCompleted && completedSubagents.length > 0" class="completed-summary mt-4 p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-lg">
      ...
      <div class="space-y-1.5">
        ...
        >
          ...
          />
          <span class="text-slate-400 w-14 shrink-0">Task {{ s.index }}:</span>
          <span class="text-slate-300 truncate flex-1">{{ s.summary || s.task }}</span>
          <Wrench class="w-3 h-3 text-slate-500 shrink-0" />
          <span class="text-xs text-slate-500 w-6 text-right shrink-0">{{ s.toolCount }}</span>
          <span class="text-xs text-slate-500 w-12 text-right shrink-0">{{ formatDuration(s.startTime, s.endTime) }}</span>
        </div>
      </div>
    </div>
  </div>
